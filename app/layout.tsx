import type { Metadata, Viewport } from "next";
import { ReactNode } from "react";

import { ActiveBatchProvider } from "@/hooks/use-active-batch";

import "./globals.css";

export const metadata: Metadata = {
  title: "Razorbook Reach Intake",
  description: "Mobile-first intake workflow for Razorbook Reach volunteers.",
};

export const viewport: Viewport = {
  width: "device-width",
  initialScale: 1,
  viewportFit: "cover",
};

export default function RootLayout({ children }: { children: ReactNode }) {
  return (
    <html lang="en">
      <head>
        {/* Inter — display font (Stitch design system) */}
        <link
          rel="preconnect"
          href="https://fonts.googleapis.com"
        />
        <link
          rel="preconnect"
          href="https://fonts.gstatic.com"
          crossOrigin="anonymous"
        />
        <link
          href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800;900&display=swap"
          rel="stylesheet"
        />
        {/* Material Symbols Outlined — icon font (Stitch design system) */}
        <link
          href="https://fonts.googleapis.com/css2?family=Material+Symbols+Outlined:opsz,wght,FILL,GRAD@20..48,100..700,0..1,-50..200&display=swap"
          rel="stylesheet"
        />
      </head>
      <body className="bg-bg-light font-display text-charcoal antialiased">
        <ActiveBatchProvider>{children}</ActiveBatchProvider>
      </body>
    </html>
  );
}
